...
const trayIconOn = join(assetPath, `tray_icon_${mode}_on.png`);
const trayIconOff = join(assetPath, `tray_icon_${mode}_off.png`);

export const toggleClickThrough = (mainWindow: BrowserWindow, controllerWindow: BrowserWindow, tray: Tray) => {
  ...
  controllerWindow.webContents.send(isOn ? types.WINDOW_TRANSPARENT_OFF : types.WINDOW_TRANSPARENT_ON);
